...
def main():
    ...
    results['Stacking Ensemble'] = evaluate_model(stacking_ensemble, X_train, y_train, X_test, y_test)
    ...
    for name, model in models.items():
        joblib.dump(model, f'{name}_best_model.pkl', compress = 3)
    joblib.dump(stacking_ensemble, 'stacking_ensemble_model.pkl', compress = 3)
    joblib.dump(selector, 'feature_selector.pkl', compress = 3)
