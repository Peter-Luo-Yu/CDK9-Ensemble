...
def feature_engineering(data, fragment_library):
    ...
    features = np.hstack((ecfp_features, np.array(fragment_similarities), np.array(fragment_matches).reshape(-1, 1)))  # Smiles + 
...
def create_hard_ensemble(models):
    return VotingClassifier(estimators=list(models.items()), voting='hard')

def cross_validate_model(model, X, y, cv=5):
    auc_scores = cross_val_score(model, X, y, cv=cv, scoring='accuracy')
...
def main():
    ...
    hard_ensemble_model = create_hard_ensemble(models)
    print("Fitting Ensemble model...")
    hard_ensemble_model.fit(X_selected, data['Active'])
    print("Cross-validating Ensemble model...")
    hard_ensemble_accuracy, hard_ensemble_precision = cross_validate_model(hard_ensemble_model, X_selected, data['Active'])
    results['Ensemble'] = {'Accuracy': hard_ensemble_accuracy, 'Precision': hard_ensemble_precision}
    
    # Save models and feature selector
    for name, model in models.items():
        joblib.dump(model, f'{name}_best_model.pkl')
    joblib.dump(hard_ensemble_model, 'ensemble_model.pkl')
