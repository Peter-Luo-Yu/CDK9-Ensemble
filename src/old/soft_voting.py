...
def cross_validate_model(model, X, y, cv=5):
    ...
    return auc_scores.mean(), precision_scores.mean(), accuracy_scores.mean()
...
def main():
    ...
    for name, model in models.items():
        ...
        results[name] = {'AUC': auc, 'Precision': precision, 'Accuracy': accuracy}
    ...
    soft_ensemble_auc, soft_ensemble_precision, soft_accuracy = cross_validate_model(soft_ensemble_model, X_selected, data['Active'])
    results['Ensemble'] = {'AUC': soft_ensemble_auc, 'Precision': soft_ensemble_precision, 'Accuracy': soft_accuracy}
